...
const getAll = asyncHandle(async (req, res) => {
  const invoices = await InVoiceModel.find()
    .select(
      '-address -fullname -email -paymentMethod -phoneNumber -fullAddress -updatedAt -__v'
    )
    .sort({ createdAt: -1 });
...
const createPaymentInvoiceTicket = asyncHandle(async (req, res) => {
  ...
  try {
    ...
    if (invoiceCreated) {
      for (const ticket of ticketsReserve) {
        ...
        if (checkTicket && checkTicket.status === 'Reserved') {
          await TicketModel.findByIdAndUpdate(checkTicket._id, {
          ...
          res.status(400);
          throw new Error(
            'Thanh toán không thành công do vé đã hết hạn thanh toán hoặc loại vé này đã bị hủy'
          );
        }
      }
    }
    ...
    const notification = new NotificationModel({
      recipientId: idUser,
      type: 'paymentTicket',
      content: `Thanh toán ${titleEvent} thành công với tổng số tiền là ${convertMoney(
        totalPrice
      )}`,
        ...
        fcmTokens.map(
          async (fcmToken) =>
            await notificationController.handleSendNotification({
              ...
              title: 'Thông báo',
              subtitle: '',
              body: `Thanh toán ${titleEvent} thành công với tổng số tiền là ${convertMoney(
                totalPrice
              )}`,
              data: {
                type: 'payment',
...
const CancelInvoice = asyncHandle(async (req, res) => {
  ...
  try {
    for (const ticket of ticketsReserve) {
      const checkTicket = await TicketModel.findById(ticket).session(session);
      if (checkTicket && checkTicket.status === 'Reserved') {
        const ticketData = await TicketModel.findByIdAndDelete(
          checkTicket._id
        ).session(session);
        await TypeTicketModel.findByIdAndUpdate(ticketData.typeTicket, {
          $inc: { amount: 1 },
        }).session(session);
      }
      // else{
